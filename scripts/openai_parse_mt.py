...
api_gate = BoundedSemaphore(10)  # Limita a 10 chamadas concorrentes
    ...
    for attempt in range(5):
        with api_gate:
            try:
                ...
                #json.loads(content)  # tenta parsear
                return resp
            except json.JSONDecodeError:
                logger.warning(
                    "JSON inválido na tentativa %d, reexecutando...", attempt + 1
                )
            except Exception as e:
                logger.warning("Erro na API (tentativa %d): %s", attempt + 1, e)
        time.sleep(delay + random.random())  # jitter
        delay = min(delay * 2, 30)
    raise RuntimeError("Falha na API após retries")


db_check = sqlite3.connect("parse_check.db", check_same_thread=False)
...
db_check.commit()
db_check.execute("PRAGMA journal_mode=WAL;")
db_check.execute("PRAGMA synchronous=NORMAL;")

parse_db_lock = Lock()
ls_db_lock = Lock()
whitaker_db_lock = Lock()
...
def consulta_whitaker_words(palavras: list[str]):
    ...
    try:
        ...
        with whitaker_db_lock:
            newline_cmd_echo = "\\n" * len(palavras)
            result = subprocess.run(
                [
                    "bash",
                    "-c",
                    f"echo -e '{newline_cmd_echo}' | bin/words {' '.join(palavras)}",
                ],
                capture_output=True,
                cwd="whitakers-words",
                text=True,
        ...
        logger.error("Erro ao executar o comando: %s", e)
        logger.error("Saída do erro: %s", e.stderr)

    return ""


conn = sqlite3.connect(DB_LS, check_same_thread=False)
...
def find_lema_in_ls_dict(lemas=[]):
    with ls_db_lock:
        c = conn.cursor()
        logger.info("Buscando lemas: %s", lemas)
...
system_prompt = """Você é um lexicógrafo escolar especializado em latim com a tarefa de corrigir o texto do dicionário Faria latim-português, com profundo conhecimento do Latim Clássico e Eclesiástico. Sua função é analisar textos extraídos de dicionários latinos, identificar e corrigir erros comuns de OCR e reescrever os verbetes de forma clara e precisa em português formal.
...
Retorne APENAS os resultados no formato TSV (Tab-Separated Values), sem cabeçalho, com UMA LINHA POR VERBETE.
A ordem das colunas deve ser exatamente:

lemas\tmorfologia\tdefinicao\texemplos\tnotas

Regras:
- lemas: lista de lemas separados por vírgula (sem colchetes ou aspas).
- morfologia: informações gramaticais, ex.: "-i, m.".
- definicao: definição completa corrigida em português formal.
- exemplos: exemplos de uso (citações a originais) separados por ponto e vírgula.
- notas: observações/correções de OCR; vazio se não houver.
- Substitua quebras de linha internas por \\n e tabulações internas por espaço.
- Não inclua texto extra, comentários, cabeçalho ou marcas fora do TSV.

Não precisa mencionar o nome do dicionário Faria na resposta, apenas reescreva os verbetes. Foque em corrigir erros, como confusões entre 'c' e 'e', 'l' e 'i', 'n' e 'u', etc. Se a definição for multilinha, adicione-a na coluna "definicao" também. Se não houver exemplos, deixe a coluna "exemplos" como uma lista vazia. Se não houver notas, deixe a coluna "notas" como uma string vazia.

Instruções:
1. Separe cada verbete individualmente por linha.
...
3. Extraia o lema, a definição, exemplos de uso (se houver), e quaisquer notas relevantes, se a explicação for multilinha, adicione-a na coluna "definicao" também. Use \\n para quebras internas.
4. Não use aspas, colchetes ou chaves.
5. Mantenha exatamente a ordem das colunas.

Exemplo de saída:
abacus\t-i, m.\t(gr. abax) mesa de mármore ou madeira, usada para cálculos. (Cic. Mil. 16; Plin. Nat. 37, 2; Varr. R.R. 1, 7)\tCic. Mil. 16; Plin. Nat. 37, 2; Varr. R.R. 1, 7\tCorrigido erro de OCR: 'abacus' estava como 'abucus'; 'abax' estava escrito como 'abaxc'.
a, ab, abs\tprep. com ablativo\tde, desde, por causa de, a partir de\tCic. Fam. 1, 9; Cic. Att. 1, 1; Cic. Phil. 2, 3\t
"""

with open("chunks_faria.json", "r") as f:
    verbetes = json.load(f)

logger.info("Total de verbetes a processar: %d", len(verbetes))
...
def find_todos_lemas(verbete: str):
    ...
    for v in verbetes:
        ...
        if lemma_next:
            for l in lema_esp:
                ...
                logger.info("Lema encontrado: %s", l)
            lemma_next = False
            continue
        ...
        logger.info("Lemas fuzzy encontrados: %s", lemas_fuzzy)
    ...
    logger.info("Lemas finais encontrados: %s", lemas)

    return set(lemas)


def db_check_verifica(doc_name, page_num):
    with parse_db_lock:
...
def processar_chunk(i, chunk):
    logger.info(
        "Processando chunk %d/%d com %d caracteres...", i + 1, len(verbetes), len(chunk)
    )

    # Verifica se o chunk já foi processado
    if db_check_verifica(f"chunk_{i+1}", i + 1):
        logger.info("Chunk %d já processado, pulando...", i + 1)
        return
    ...
    mensagem_user += "\n\nAgora, pense em alguns passos os relacionamentos possíveis entre os verbetes acima do LS e os verbetes abaixo do dicionário Faria, corrigindo possíveis erros de OCR (do Faria) e estruturando-o e informando apenas TSV (Tab-Separated Values) de resultado. Verbetes do Faria:\n\n"
    mensagem_user += chunk

    logger.info("Mensagem do usuário: %s", mensagem_user)
    ...
    logging.info("Resposta do chunk %d: %s", i + 1, resposta_texto.strip())
    # try:
    #     resposta_json = json.loads(resposta_texto)
    #     logger.info(
    #         "Resposta JSON do chunk %d: %s",
    #         i + 1,
    ...
    #     logger.error("Erro ao decodificar JSON na resposta do chunk %d: %s", i + 1, e)
...
with ThreadPoolExecutor(max_workers=12) as executor:
    ...
    for i, chunk in enumerate(verbetes):
        futures.append(executor.submit(processar_chunk, i, chunk))
    for future in futures:
        try:
            future.result()
        except Exception as e:
            logger.error("Erro ao processar chunk: %s", e)
